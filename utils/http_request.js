...
export default async function httpRequest(url, options, reqId = "http-request", expectStatusCode = 200) {
  try {
    ...
    console.log(`${logTime(reqId)} HTTP ${options.method} request sent to '${url}'`);
    ...
    if (expectStatusCode.toString().indexOf(response.statusCode.toString()) !== -1) {
      // Response good
      console.log(
        `${logTime(reqId)} HTTP ${options.method} response received from '${url}' with status code '${response.statusCode}'`,
      );
      return response;
    }
    // Response bad
