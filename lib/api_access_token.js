...
export default async function getToken(forceRegenerate = false) {
  const now = Date.now();

  // Return access token
  if (accessToken.token && accessToken.expires > now && !forceRegenerate) {
  ...
  if (refreshToken.token && refreshToken.expires > now && !forceRegenerate) {
